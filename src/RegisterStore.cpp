...
void RegisterStore::initializeDefinitions()
{
    ...
    defineRegister({kAddrSystemStatus, QStringLiteral("系统状态位"), RegisterKind::Holding, 1, RegisterFormat::U16, false, true,
                    QStringLiteral("0=无报警，1=超限报警，2=反流报警，3=超量程报警。"), false});
    defineRegister({kAddrFlowDisplay, QStringLiteral("流量"), RegisterKind::Holding, 2, RegisterFormat::Float, false, true,
                    QStringLiteral("浮点数（低字在前、高字在后），屏幕显示值，为负时显示0。"), false});
    defineRegister({kAddrPowerOnTotal, QStringLiteral("上电累计"), RegisterKind::Holding, 2, RegisterFormat::Float, true, true,
                    QStringLiteral("浮点数（低字在前、高字在后），屏幕显示值。"), false});
    defineRegister({kAddrTemperature, QStringLiteral("温度"), RegisterKind::Holding, 2, RegisterFormat::Float, false, true,
                    QStringLiteral("浮点数（低字在前、高字在后），屏幕显示值。"), false});
    defineRegister({kAddrCalibrationTablePoints, QStringLiteral("表格(8个点)"), RegisterKind::Holding, 16, RegisterFormat::Raw, true, true,
                    QStringLiteral("校准点值。"), false});
    defineRegister({kAddrCalibrationTableCoefficients, QStringLiteral("表格(8个修正系数)"), RegisterKind::Holding, 16, RegisterFormat::Raw, true, true,
                    QStringLiteral("校准系数。"), false});
...
void RegisterStore::initializeDefaults()
{
    ...
    setReg(kAddrSystemStatus, 0);
    setFloat(kAddrFlowDisplay, 12.34f);
    setFloat(kAddrPowerOnTotal, 1234.56f);
    setFloat(kAddrTemperature, 25.0f);
    ...
    setReg(kAddrTemperatureEnable, 1);
}
